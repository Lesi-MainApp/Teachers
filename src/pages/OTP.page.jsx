...
const OTPPage = () => {
  ...
  const [verifyPhoneOtp, { isLoading: verifying }] = useVerifyPhoneOtpMutation();
  const [resendVerifyOtp, { isLoading: resending }] = useResendVerifyOtpMutation();
  const [forgotSendOtp, { isLoading: sendingForgotOtp }] = useForgotPasswordSendOtpMutation();
  ...
  const handleVerify = async () => {
    ...
    try {
      ...
      // forgot flow: we do NOT verify here because backend verifies inside reset endpoint.
      // So we just move to reset screen with phone+code.
      navigate(
        `/forgotpassword?phone=${encodeURIComponent(phone)}&code=${encodeURIComponent(code)}`
      );
    } catch (err) {
